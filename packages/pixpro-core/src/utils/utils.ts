 ...
 * 给 CSS 添加作用域选择器
 */
export function scopeStyles(styles: string | any, scopeId: string): string {
  // 当 styles 是 CSS Modules 对象时，试图获取 toString 方法或 cssText 等属性
  if (typeof styles !== 'string') {
    if (styles.toString && typeof styles.toString === 'function') {
      return styles.toString().replace(/\.photo-studio-container/g, 
        `.photo-studio-container[data-style-id="${scopeId}"]`);
    }
    if (styles.cssText) {
      return styles.cssText.replace(/\.photo-studio-container/g, 
        `.photo-studio-container[data-style-id="${scopeId}"]`);
    }
    // 如果无法处理，返回空字符串
    console.warn('Cannot process styles: not a string or compatible object');
    return '';
  }
